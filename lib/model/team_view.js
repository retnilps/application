  ...
  if (user.is_admin() || user.company.share_all_absences) {
    ...
    promise_departments = user.company.getDepartments()
      .then(function (departments) {
        // Remove Board Office from shared calendar
        if (user.getDepartment().name !== config.get('board_department')) {
          departments = departments.filter(function( department ) {
            return department.name !== config.get('board_department');
          });
        }
        return normalise_departments_func(null, departments);
      });
      //if (!user.is_admin()) {
      //  console.log(promise_departments[0].name)//then(function (departments) { console.log(departments[0].name)});
      //  promise_departments = _.without(promise_departments, 'direzione');
      //}
