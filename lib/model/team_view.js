...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  if (user.is_admin() || user.company.share_all_absences) {
    ...
    promise_departments = user.getDepartment().then(function(user_department) {
        return user.company.getDepartments()
          .then(function (departments) {
            console.log(user_department.name)
            // Remove Board Office from shared calendar
            if (user_department.name !== config.get('board_department')) {
